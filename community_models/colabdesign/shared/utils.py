    ...
    for k,v in x.items():
      if v is not None:
        if k in d:
          ...
          print(f"colabdesign.shared.utils line 34 ERROR: '{k}' not found in {list(d.keys())}")  
...
def custom_prng_key(seed=None, device=None):
  """
  Creates a JAX PRNG key similar to jax.random.PRNGKey().

  Args:
    seed: Optional integer seed. If None, a random seed will be generated.

  Returns:
    A JAX PRNG key (jax.Array) with shape (2,)
  """
  if seed is None:
    # Generate a random seed if none provided
    seed = random.randint(0, 2**32 - 1)

  # Convert seed to uint32 array of shape (2,)
  # This matches JAX's internal key format
  key = np.array([seed, 0], dtype=np.uint32)

  # Convert to JAX array
  return jax.device_put(key, device)

class Key():
  '''random key generator'''
  def __init__(self, key=None, seed=None, device=None):
    if key is None:
      self.seed = random.randint(0,2147483647) if seed is None else seed
      # self.key = jax.random.PRNGKey(self.seed) 
      self.key = custom_prng_key(self.seed, device)
    else:
      self.key = key
  def get(self, num=1):
    if num > 1:
