...
class mk_af_model(design_model, _af_inputs, _af_loss, _af_prep, _af_design, _af_utils):
  def __init__(self,
    ...
    assert len(self._model_params) > 0, "No model parameters found"
    print(f"Found {len(self._model_params)} model parameters")
    print(f"Model names: {self._model_names}")
  ...
  def _get_model(self, cfg, callback=None):
    ...
    return {"grad_fn":jax.jit(jax.value_and_grad(_model, has_aux=True, argnums=0), device=device),
            "fn":jax.jit(_model, device=device), "runner":runner}
