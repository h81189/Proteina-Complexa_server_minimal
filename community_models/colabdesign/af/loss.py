...
class _af_loss:
  ...
  def _loss_binder(self, inputs, outputs, aux):
    ...
    # unsupervised losses
    complex_ipsae = get_ipsae_loss(outputs, mask_1d=binder_id, mask_1b=target_id) # default is 15.0 which is standard for af2
    complex_ipsae_10 = get_ipsae_loss(outputs, mask_1d=binder_id, mask_1b=target_id, pae_cutoff=10.0)
    aux["losses"].update({
      ...
      "min_ipae": get_min_ipae_loss(outputs, mask_1d=binder_id, mask_1b=target_id),
      # "min_ipsae": get_min_ipsae_loss(outputs, mask_1d=binder_id, mask_1b=target_id),
      "min_ipsae": complex_ipsae["min"],
      "max_ipsae": complex_ipsae["max"],  
      "avg_ipsae": complex_ipsae["avg"],
      "min_ipsae_10": complex_ipsae_10["min"],
      "max_ipsae_10": complex_ipsae_10["max"],
      "avg_ipsae_10": complex_ipsae_10["avg"],
    })
...
def mask_min_loss(x, mask=None):
  if mask is None:
    return x.min()
  else:
    x_masked = x * mask + 1e9 * (1 - mask)
    return x_masked.min()
...
def get_min_ipae_loss(outputs, mask_1d=None, mask_1b=None, mask_2d=None):
  p = get_pae(outputs) / 31.0
  # p = (p + p.T) / 2
  L = p.shape[0]
  if mask_1d is None: mask_1d = jnp.ones(L)
...
def get_min_ipsae_loss(outputs, mask_1d, mask_1b, pae_cutoff=15.0):
  ...
  # B -> A
  pae_mask_2d = mask_1b[:, None] * mask_1d[None, :] * (p < pae_cutoff)    # (L, L)
  d0 = calc_d0(pae_mask_2d.sum(axis=1))                                  # (L)
  tm_term = 1. / (1 + jnp.square(p) / jnp.square(d0)[:, None])          # (L, L)
  ...
  # A -> B
  pae_mask_2d = mask_1d[:, None] * mask_1b[None, :] * (p < pae_cutoff)    # (L, L)
  d0 = calc_d0(pae_mask_2d.sum(axis=1))                                  # (L)
  tm_term = 1. / (1 + jnp.square(p) / jnp.square(d0)[:, None])          # (L, L)
  mean_tm_term = (pae_mask_2d * tm_term).sum(axis=1) / (pae_mask_2d.sum(axis=1) + 1e-8)
  ipsae_ab = mean_tm_term.max()

  return jnp.minimum(ipsae_ab, ipsae_ba)

def get_ipsae_loss(outputs, mask_1d, mask_1b, pae_cutoff=15.0):
  # pae cutoff 15 for af2, 10 for af3 and boltz
  p = get_pae(outputs)
  L = p.shape[0]
  ...
  complex_ipsae = {
    'min': jnp.minimum(ipsae_ab, ipsae_ba),
    'max': jnp.maximum(ipsae_ab, ipsae_ba),
    'avg': jnp.mean(jnp.array([ipsae_ab, ipsae_ba]))
  }
