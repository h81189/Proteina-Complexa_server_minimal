...
from proteinfoundation.datasets.atomworks_dataloader_utils import atomworks_pad_batch

# from modelhub.resolvers import register_resolvers
#! TODO bring this back
from proteinfoundation.datasets.atomworks_ddp_utils import RankedLogger
...
register_resolvers()  # register the resolvers
...
def wrap_dataset_and_sampler_with_fallbacks(
    ...
    fallback_sampler = LazyWeightedRandomSampler(
        weights=(
            sampler_to_fallback_to.weights
    ...
    wrapped_dataset = FallbackDatasetWrapper(dataset_to_be_wrapped, fallback_dataset=dataset_to_fallback_to)
...
def instantiate_single_dataset_and_sampler(cfg: DictConfig | dict) -> dict[str, Any]:
    ...
    if "weights" in cfg and "sampler" not in cfg:
        ...
        sampler = WeightedRandomSampler(
            ...
            num_samples=len(dataset),
            replacement=True,  #! we want False
        )

    return {"dataset": dataset, "sampler": sampler}


def recursively_instantiate_datasets_and_samplers(cfg: DictConfig | dict, name: str | None = None) -> dict[str, Any]:
    ...
    Returns:
        dict[str, Any]: A dictionary containing the instantiated dataset and sampler.
    """
    #! TODO figure out how to get dataloader into lightning
    #! for PDB only can remove the sub datasets but for mixture will likely need to figure out
    ...
    # ------- Recursive case (2): Multiple sub-datasets that must be concatenated together -------#
        ...
        for sub_dataset_name, sub_dataset_cfg in cfg.sub_datasets.items():
            ...
            datasets_info.append(recursively_instantiate_datasets_and_samplers(sub_dataset_cfg, name=sub_dataset_name))
        ...
        concatenated_dataset = ConcatDatasetWithID(datasets=[info["dataset"] for info in datasets_info])
        concatenated_weights = torch.cat([info["sampler"].weights for info in datasets_info])
        sampler = WeightedRandomSampler(
    ...
    # ------- Recursive case (3): Multiple datasets that must be sampled from with specified probabilities -------#
        ...
        for nested_dataset_name, nested_dataset_cfg in cfg.items():
            ...
            assert "probability" in nested_dataset_cfg, "Expected 'probability' key in dataset configuration"
            datasets_info.append(
                {
                    **recursively_instantiate_datasets_and_samplers(nested_dataset_cfg, name=nested_dataset_name),
                    "probability": nested_dataset_cfg["probability"],
                }
            )

        # ... check that the sum of probabilities of all datasets is 1
        assert abs(1 - sum(dataset_info["probability"] for dataset_info in datasets_info)) < 1e-5, (
...
def simple_dataloader(
    ...
    return DataLoader(
        dataset=dataset,
        collate_fn=lambda x: atomworks_pad_batch(x),
...
def assemble_distributed_loader(
    ...
    #! sampler created upstream so for now forcing None to debug easier
    sampler = None
    rank = None
    world_size = None
    print("\n\n\n FORCING SAMPLER TO NONE TO TEST LOCALLY")
    if isinstance(sampler, MixedSampler):
        # (If given a MixedSampler, we must convert to a DistributedMixedSampler)
        assert rank is not None and world_size is not None and n_examples_per_epoch is not None, (
    ...
    elif sampler is None:  #! and isinstance(dataset, Subset): # trying off
    ...
    if "n_fallback_retries" in loader_cfg and loader_cfg.n_fallback_retries > 0 and sampler is not None:
        ranked_logger.info(f"Wrapping train dataset and sampler with {loader_cfg.n_fallback_retries} fallbacks...")
    ...
    if "dataloader_params" in loader_cfg:
        print(f"\n\nDataloader params: {loader_cfg.dataloader_params}")
    # ... assemble the final loader
    loader = DataLoader(
        dataset=dataset,
        sampler=None,  # sampler,
        collate_fn=lambda x: atomworks_pad_batch(x),  # No collation
        **loader_cfg.dataloader_params if "dataloader_params" in loader_cfg else {},
    )
...
def assemble_val_loader_dict(
    ...
    for val_dataset_name, val_dataset in cfg.items():
        ...
        assert "dataset" in val_dataset, f"Expected 'dataset' key in validation dataset config for {val_dataset_name}"
        dataset = hydra.utils.instantiate(val_dataset.dataset)  # directly instantiate the dataset
        ...
        if "key_to_balance" in val_dataset and val_dataset.key_to_balance:
            ...
            assert key_to_balance in dataset.data.columns, f"Key {key_to_balance} not found in dataset columns!"
