...
def _import_progress_bar():
    ...
    from proteinfoundation.cli.progress_bar import (
        clear_pipeline_step,
        set_pipeline_step,
    )
...
def _import_target_manager():
    ...
    from proteinfoundation.cli.target_manager import (
        add_target_cli,
...
class StepConfig:
    ...
    @classmethod
    def from_yaml(
        cls, config_path: Path, overrides: list[str] | None = None
    ) -> StepConfig:
            ...
            try:
                ...
                if "generation" in cfg and isinstance(cfg["generation"], dict):
                    ...
                    if task_name and not str(task_name).startswith("${"):
                        ...
                        config.target_name = _get_task_name_from_dataloader(
                            gen_cfg["dataloader"]
                        )
        ...
        if override_task_name:
            ...
            if "generation" in cfg and isinstance(cfg["generation"], dict):
                ...
                # Fall back to dataloader.dataset.task_name
                    ...
                    summary["target_name"] = _get_task_name_from_dataloader(
                        gen_cfg["dataloader"]
                    )
        ...
        if DEFAULT_INFERENCE_DIR.exists():
            ...
            if matching_dirs:
                ...
                summary["n_pdbs"] = len(
                    [
                        p
                        for p in output_dir.rglob("*.pdb")
                        if not p.name.endswith("_binder.pdb")
                    ]
                )
    except Exception:
...
def run_step(
    ...
    use_parallel_execution = (
        stage_njobs > 1 and not config.verbose and step_name in {"generate", "evaluate"}
    )
    ...
    if config.log_file is None and not config.verbose:
        config.log_file = create_log_file(
            step_name, run_name=config.run_name, target_name=config.target_name
        )
    ...
    try:
        if use_parallel_execution:
            if design_parallel:
                stage_log_dir = config.pipeline_log_dir or (
                    config.log_file.parent if config.log_file else LOG_DIR
                )
...
def run_design_pipeline(
    ...
    os.environ["TARGET_NAME_HK"] = str(config.target_name)
    ...
    # debug here
    job = next(
        (j for j in r.json() if j["job_id"] == os.environ.get("JOB_ID_HK")), None
    )
    # print(job)
    # run_id = job['run_id']
    global run_id

    global target_name_hk
    # run_id=668
    while True:
        r = requests.patch(
...
def handle_status(args: argparse.Namespace) -> None:
    ...
    if DEFAULT_EVAL_DIR.exists():
        ...
        print(
            f"    ✓ ./evaluation_results/ ({len(eval_dirs)} runs, {len(eval_csvs)} CSVs)"
        )
    else:
...
def handle_target(args: argparse.Namespace) -> None:
    ...
    add_target_cli, add_target_interactive, list_targets, show_target = (
        _import_target_manager()
    )
...
def handle_validate(args: argparse.Namespace) -> None:
    ...
    if (
        validation_type in {"generate", "evaluate", "analyze", "design"}
        and not config_path
    ):
...
def main() -> None:
    ...
    debug_hk = False
    if not debug_hk:
        job = next((j for j in r.json() if j["job_id"] == sys.argv[3]), None)
        if job is None:
            print(r.json(), sys.argv[3])
            print("no jobs matching ID")
            sys.exit(1)
        os.environ["JOB_ID_HK"] = str(job["job_id"])
    ...
    os.environ["RUN_ID_HK"] = str(run_id)
    ...
    r = requests.get(
        f"{API_BASE}/uploads/structures/"
        + os.path.basename(config.split("\n")[3].split(" ")[-1])
    )
    r.raise_for_status()
    with open(
        config.split("\n")[3].split(" ")[-1], "wb"
    ...
    # ['/workspace/.venv/bin/complexa', 'design', 'configs/search_binder_local_pipeline.yaml', '++run_name=quick_test', '++generation.task_name=02_PDL1', '++generation.args.nsteps=100', '++generation.dataloader.dataset.nres.nsamples=2'] # example quick run argv (with overrides)
    sys.argv = [
        "/workspace/.venv/bin/complexa",
        "design",
        "configs/search_binder_local_pipeline.yaml",
        "--verbose",
        "++generation.task_name=" + config.split(":")[0],
        "++ckpt_path=/home/hari.koneru/Proteina-Complexa/ckpts",
        "++autoencoder_ckpt_path=/home/hari.koneru/Proteina-Complexa/ckpts/complexa_ae.ckpt",
    ]
    for k in cli.keys():  # load any other design settings as overrides
        if k == "binder_length":
            continue
        sys.argv.append("++" + k + "=" + str(cli[k]))

    print(config)
    print(cli)
    # config[target].pop('out_dir', None)
    with open("configs/targets/targets_dict.yaml", "a") as f:
        f.write(
            "\n\n  " + config.replace("\n", "\n  ")
        )  # save modified config with indent
...
def download_main(extra_args: list[str] | None = None) -> None:
    ...
    script_path = (
        Path(__file__).parent.parent.parent.parent / "env" / "download_startup.sh"
    )
