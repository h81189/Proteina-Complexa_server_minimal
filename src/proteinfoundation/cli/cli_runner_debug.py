...
def run_design_pipeline(
    ...
    # pdb.set_trace()
    try:
        r = requests.get(f"{url2}/jobs/active") # find this job
        r.raise_for_status()
        API_BASE = url2
    except:
        r = requests.get(f"{url1}/jobs/active") # find this job
        r.raise_for_status()
        API_BASE = url1
    ...
    r = requests.patch(f"{API_BASE}/jobs/{os.environ.get('JOB_ID_HK')}/update", json={ # update job data
        "status": "Finished",
    })
    r.raise_for_status()
    r = requests.patch(f"{API_BASE}/runs/{run_id}/progress", json={
...
def main() -> None:
    ...
    config=config[:len_ind]+'\n  binder_length: '+str(cli['binder_length'])+config[len_ind:]

    r = requests.get(f"{API_BASE}/uploads/"+config.split("\n")[2].split(" ")[-1]+".pdb")
    r.raise_for_status()
    with open(config.split("\n")[3].split(" ")[-1], 'wb') as f: # built-in full name/hardcoded path
        f.write(r.content)
    ...
    sys.argv=['/workspace/.venv/bin/complexa', 'design', 'configs/search_binder_local_pipeline.yaml', '--verbose', '++generation.task_name='+config.split(":")[0], '++ckpt_path=/home/hari.koneru/Proteina-Complexa/ckpts', '++autoencoder_ckpt_path=/home/hari.koneru/Proteina-Complexa/ckpts/complexa_ae.ckpt']
    ...
    import pdb
    pdb.set_trace()
    config='CD274_IgV_l1:\n  source: HK\n  target_filename: 20260312205843_pdl1_pxdesign\n  target_path: /home/hari.koneru/all_targets/20260312205843_pdl1_pxdesign.pdb\n  target_input: A17-132\n  hotspot_residues: []\n  binder_length: [65, 150]\n  pdb_id: null\n'
