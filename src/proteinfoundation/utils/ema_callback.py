...
# modified from https://github.com/NVIDIA/NeMo/blob/main/nemo/collections/common/callbacks/ema.py
...
class EMAOptimizer(torch.optim.Optimizer):
    ...
    def switch_main_parameter_weights(self, saving_ema_model: bool = False):
        ...
        for param, ema_param in zip(self.all_parameters(), self.ema_params, strict=False):
...
class EmaModelCheckpoint(ModelCheckpoint):
    """
    Extends pytorch's lightning ModelCheckpoint callback by storing two checkpoints, one with
    the regular trained weights and another with the EMA weights. The latter one is stored
    with the same name but replacing the extension `.ckpt` by `-EMA.ckpt`.

    Storing dual checkpoints works, but may be unnecessary. Because the regular checkpoint
    also has the EMA weights in it, but not as the model weights but as part of the optimizer
    state. Loading a (regular) checkpoint these EMA weights can be accessed by

    ckpt = torch.load(ckpt_file)
    ema_params = ckpt["optimizer_states"][0]["ema"]

    where `ema_params` is a tuple with all EMA model params.

    The original weights in that same checkpoint are in

    ckpt = torch.load(ckpt_file)
    reg_params = ckpt["state_dict"]

    where `reg_params` is a dictionary with all regular model params.

    There are some ways to swap weights, implemented in this file. For now we use the two
    checkpoint system which is easier. Later we may move to single regular checkpoint and]
    swapping weights ourselves if that's better.
    """
    ...
    def _ema_callback(self, trainer: "pl.Trainer") -> EMA | None:
        ema_callback = None
