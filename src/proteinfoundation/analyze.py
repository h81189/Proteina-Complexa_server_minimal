...
from proteinfoundation.result_analysis.compute_diversity import (
    compute_foldseek_diversity,
    compute_mmseqs_diversity,
)
...
def save_transposed_csv(df: pd.DataFrame, original_csv_path: str) -> None:
    ...
    try:
        ...
        df_transposed.columns = [
            f"sample_{i}" for i in range(len(df_transposed.columns))
        ]
...
def add_sequence_columns(
    ...
    if pdb_path_column not in df.columns:
        logger.warning(
            f"Column '{pdb_path_column}' not found in DataFrame, skipping sequence extraction"
        )
        return df
    ...
    if result_type in ["protein_binder", "motif_protein_binder"]:
        ...
        logger.info(
            f"Extracted {n_binder} binder sequences and {n_target} target sequences"
        )
        seq_cols = ["binder_sequence", "target_sequence"]
    elif result_type in ["ligand_binder", "motif_ligand_binder"]:
        n_binder = df["binder_sequence"].notna().sum()
        logger.info(
            f"Extracted {n_binder} binder sequences (ligand targets have no protein sequence)"
        )
...
def validate_config(cfg: DictConfig, results_dir: str = None) -> None:
    ...
    if result_type not in VALID_RESULT_TYPES:
        raise ValueError(
            f"Invalid result_type '{result_type}'. Valid options: {VALID_RESULT_TYPES}"
        )
...
def print_dryrun_summary(
    ...
    logger.info(
        f"  Sequence types: {cfg_aggregation.get('sequence_types', ['self', 'mpnn', 'mpnn_fixed'])}"
    )
...
def find_result_files(
    ...
    logger.info(
        f"Found {len(result_files)} result files matching pattern: {base_pattern}[0-9]+.csv"
    )
...
def merge_monomer_into_binder(
    ...
    if "pdb_path" not in binder_df.columns:
        logger.warning(
            "binder_df has no 'pdb_path' column, cannot merge monomer results"
        )
        return binder_df

    monomer_files = _find_monomer_results_for_binder(
        results_dir, config_name, input_mode
    )
    ...
    logger.debug(
        f"Loaded {len(monomer_df)} monomer results from {len(monomer_dfs)} files"
    )
    ...
    if not monomer_only_cols:
        logger.info(
            "No unique monomer columns to merge (all columns already in binder results)"
        )
        return binder_df
...
def merge_monomer_into_motif(
    ...
    if "pdb_path" not in motif_df.columns:
        logger.warning(
            "motif_df has no 'pdb_path' column, cannot merge monomer results"
        )
        return motif_df
    ...
    if not unique_cols and not conflict_cols:
        logger.debug(
            "No new monomer metric columns to merge (all columns are metadata)"
        )
        return motif_df
    ...
    monomer_subset = (
        monomer_df[merge_cols_from_monomer]
        .rename(columns=rename_map)
        .drop_duplicates(subset=["pdb_path"])
    )
...
def detect_sequence_types_from_columns(df: pd.DataFrame) -> list[str]:
    ...
    if result:
        ...
        logger.warning(
            "Could not auto-detect sequence types from columns, using defaults"
        )
...
def get_groupby_columns(df: pd.DataFrame) -> list[str]:
    ...
    if (
        "task_name" in groupby_cols
        and "task_name" in df.columns
        and df["task_name"].isna().all()
    ):
    ...
    logger.info(
        f"  Groupby cols:  {', '.join(groupby_cols[:5])}{'...' if len(groupby_cols) > 5 else ''}"
    )
    return groupby_cols
...
def aggregate_interface_metrics_for_successful_samples(
    ...
    for seq_type, df_successful in successful_dfs.items():
        try:
            if len(df_successful) == 0:
                logger.info(
                    f"No successful samples for {seq_type}, skipping interface metrics"
                )
                continue
            ...
            df_clean = df_successful[existing_metrics].replace(
                [-1, float("inf"), float("-inf")], pd.NA
            )
            ...
            if groupby_cols:
                ...
                if not valid_groupby:
                    logger.warning(
                        f"No valid groupby columns for {seq_type}, using global means"
                    )
            ...
            if groupby_cols_to_use:
                ...
                df_for_agg = pd.concat(
                    [df_successful[groupby_cols_to_use], df_clean], axis=1
                )
                ...
                for m in existing_metrics:
                    ...
                    rename_counts[m] = (
                        f"_res_{clean_name}_successful_{seq_type}_n_valid"
                    )
                ...
                for metric in existing_metrics:
                    ...
                    if len(valid_values) > 0:
                        result_dict[f"_res_{clean_name}_successful_{seq_type}_mean"] = (
                            valid_values.mean()
                        )
                        result_dict[
                            f"_res_{clean_name}_successful_{seq_type}_n_valid"
    ...
    if all_seq_type_dfs:
        ...
        if groupby_cols:
            valid_groupby = [
                c for c in groupby_cols if c in all_seq_type_dfs[0].columns
            ]
            if valid_groupby:
...
def compute_secondary_structure(
    ...
    if has_precomputed:
        df_grouped = (
            df.groupby(groupby_cols, dropna=False)[ss_cols].mean().reset_index()
        )
        # Rename columns with metric_suffix
        df_grouped = df_grouped.rename(
            columns={col: f"{col}_{metric_suffix}" for col in ss_cols}
        )
    elif "pdb_path" in df.columns:
        # Fallback: compute from PDB files directly
        from proteinfoundation.metrics.structural_metric_ss_ca_ca import (
            compute_ss_metrics,
        )

        logger.debug("SS columns not found, falling back to computing from PDB files")
        df_grouped = (
            df.groupby(groupby_cols, dropna=False)["pdb_path"].agg(list).reset_index()
        )
    ...
    df_grouped.to_csv(
        csv_path, sep=SEP_CSV_PD, index=False, float_format=FLOAT_FORMAT_PD
    )
...
def compute_residue_type_distribution(
    ...
    for aa_3letter, aa_1letter in restype_3to1.items():
        df_grouped[f"_res_aa_prop_{aa_1letter}_{metric_suffix}"] = [
            p[aa_3letter] for p in all_proportions
        ]
...
def _safe_merge_dfs(left: pd.DataFrame, right: pd.DataFrame) -> pd.DataFrame:
    ...
    common = list(
        set(left.columns)
        & set(right.columns) - {c for c in right.columns if c.startswith("_res_")}
    )
    if not common:
...
def compute_binder_overall_metrics(
    ...
    logger.info(
        f"  Thresholds:    {'; '.join(thresh_parts[:3])}{'...' if len(thresh_parts) > 3 else ''}"
    )
    ...
    compute_pass_rate_func = (
        compute_filter_pass_rate
        if result_type == "protein_binder"
        else compute_filter_ligand_pass_rate
    )
    ...
    try:
        ...
        if not aggregated_metrics_df.empty:
            aggregated_csv_path = os.path.join(
                results_dir, f"aggregated_interface_metrics_{config_name}.csv"
            )
    ...
    if overall_results:
        ...
        overall_csv_path = os.path.join(
            results_dir, f"overall_binder_performance_{config_name}_aggregated.csv"
        )
    ...
    r = requests.get(f"{API_BASE}/jobs/active")  # find this job
    r.raise_for_status()

    run_id = int(os.environ.get("RUN_ID_HK"))
    target_name = os.environ.get("TARGET_NAME_HK")
    try:
        t = int(open("/tmp/traj.txt").read())
    except:
        t = 0
    traj_add = len(df) - t
    t += traj_add
    open("/tmp/traj.txt", "w").write(str(t))
    r = requests.patch(
        f"{API_BASE}/runs/{run_id}/progress",
        json={
            "current_iterations": traj_add,
        },
    )
    r.raise_for_status()
    print("completed " + str(traj_add) + " iterations")
    all_binders = requests.get(f"{API_BASE}/runs/{run_id}/binders")
    r.raise_for_status()
    all_binders = [b["binder_sequence"] for b in all_binders.json()["binders"]]
    binder_seqs = successful_dfs["self"]["binder_sequence"].tolist()
    traj_id = str(random.randint(0, int(1e12 - 1))).zfill(11)
    for binder in range(len(binder_seqs)):
        if binder_seqs[binder] in all_binders:
            print("ignoring repeat: " + binder_seqs[binder])
            continue
        r = requests.patch(
            f"{API_BASE}/runs/{run_id}/progress",
            json={
        ...
        r.raise_for_status()
        binder_id = target_name + "_" + traj_id + "_" + str(binder)
        print(binder_id + ": " + binder_seqs[binder])
        r = requests.post(
            f"{API_BASE}/runs/{run_id}/binders",
            json={
                "binder_id": binder_id,
                "binder_sequence": binder_seqs[binder],
            },
        )
        r.raise_for_status()
...
def run_binder_analysis(
    ...
    if sequence_types is not None:
        sequence_types = (
            list(sequence_types)
            if hasattr(sequence_types, "__iter__")
            else [sequence_types]
        )
    ...
    for group in threshold_groups:
        ...
        for mode, models in thresholds.items():
            for model, spec in models.items():
                threshold = (
                    spec.get("threshold", 2.0) if isinstance(spec, dict) else spec
                )
                ...
                if len(df_filtered) > 0:
                    ...
                    logger.debug(
                        f"No samples passed {metric_type} {mode}/{model} {op} {threshold}"
                    )
    return subsets
...
def build_column_filtered_subsets(
    ...
    for filt in column_filters:
        ...
        if len(df_filtered) > 0:
            ...
            logger.debug(
                f"Filter {col} {op} {threshold}: {len(df_filtered)}/{len(df)} passed"
            )
        else:
...
def merge_and_save_results(
    ...
    if all_pass_rate_dfs:
        ...
        path = os.path.join(
            results_dir, f"overall_{prefix}_pass_rates_{config_name}.csv"
        )
    ...
    if all_diversity_dfs:
        ...
        merged_div = merged_div.drop(
            columns=[
                c
                for c in merged_div.columns
                if c.startswith("_res") and not c.startswith("_res_diversity")
            ],
            errors="ignore",
        )
        path = os.path.join(
            results_dir, f"overall_{prefix}_diversity_{config_name}.csv"
        )
...
def run_monomer_analysis(
    ...
    ca_codesignability_thresholds = cfg_aggregation.get(
        "ca_codesignability_thresholds", None
    )
    allatom_codesignability_thresholds = cfg_aggregation.get(
        "allatom_codesignability_thresholds", None
    )
    require_all = cfg_aggregation.get("require_all_thresholds", False)

    def _get_thresholds(
        cfg_value, metric_type: str, mode_filter: str = None, default: dict = None
    ):
        ...
        for mode, models in thresholds.items():
            for model, spec in models.items():
                if isinstance(spec, dict):
                    parts.append(
                        f"{mode}/{model} {spec.get('op', '<=')} {spec.get('threshold', 2.0)}"
                    )
                else:
    ...
    logger.info(
        f"    Designability ({des_src}):          {_fmt_monomer_thresh(designability_thresholds)}"
    )
    logger.info(
        f"    CA codesignability ({ca_codes_src}):    {_fmt_monomer_thresh(ca_codesignability_thresholds)}"
    )
    logger.info(
    ...
    def _save_monomer_csv(df_to_save: pd.DataFrame, filename: str):
        ...
        df_filtered.to_csv(
            output_path, sep=SEP_CSV_PD, index=False, float_format=FLOAT_FORMAT_PD
        )
    ...
    if designability_thresholds:
        for metric_type, label, key_prefix in [
            ...
            if not pass_df.empty:
                _save_monomer_csv(
                    pass_df, f"res_monomer_{key_prefix}_pass_rates_all_samples.csv"
                )
    ...
    if codesignability_thresholds:
        ...
        if not codes_pass_df.empty:
            _save_monomer_csv(
                codes_pass_df, "res_monomer_codesignability_pass_rates_all_samples.csv"
            )
    ...
    if "L" in df.columns:
        ...
        if designability_thresholds:
            for metric_type, label, key_prefix, by_length_fn in [
                ...
                if not by_length_df.empty:
                    _save_monomer_csv(
                        by_length_df, f"res_monomer_{key_prefix}_by_length.csv"
                    )
        ...
        if codesignability_thresholds:
            ...
            if not codes_by_length_df.empty:
                _save_monomer_csv(
                    codes_by_length_df, "res_monomer_codesignability_by_length.csv"
                )
    ...
    subset_summary = ", ".join(
        f"{s['suffix']}({len(s['df'])})" for s in monomer_subsets
    )
    ...
    # Secondary structure
    ss_cols_present = all(
        col in df.columns for col in ["_res_ss_alpha", "_res_ss_beta", "_res_ss_coil"]
    )
    ...
    if "pdb_path" in df.columns and cfg_aggregation.get(
    ...
    if novelty_cols:
        ...
        for col in novelty_cols:
            df_novelty = (
                df.groupby(groupby_cols, dropna=False)[col].mean().reset_index()
            )
    ...
    thresholds_path = os.path.join(
        results_dir, f"monomer_thresholds_{config_name}.json"
    )
...
def run_motif_analysis(
    ...
    # --- Thresholds ---
    motif_rmsd_thresh = _cfg_or_default(
        "motif_rmsd_thresholds", DEFAULT_MOTIF_RMSD_THRESHOLDS
    )
    motif_seq_rec_thresh = _cfg_or_default(
        "motif_seq_rec_threshold", DEFAULT_MOTIF_SEQ_REC_THRESHOLD
    )
    des_thresh = resolve_thresholds(
        _cfg_or_default(
            "designability_thresholds", DEFAULT_MOTIF_DESIGNABILITY_THRESHOLDS
        ),
        model=folding_model,
    )
    codes_thresh = resolve_thresholds(
        _cfg_or_default(
            "codesignability_thresholds", DEFAULT_MOTIF_CODESIGNABILITY_THRESHOLDS
        ),
        model=folding_model,
    )
            ...
            if isinstance(v, dict):
                for k2, v2 in v.items():
                    if isinstance(v2, dict):
                        parts.append(
                            f"{k}/{k2} {v2.get('op', '<=')} {v2.get('threshold', '?')}"
                        )
                    else:
                        parts.append(
                            f"{k} {v.get('op', '<=')} {v.get('threshold', '?')}"
                        )
                        break
            else:
                parts.append(f"{k}: {v}")
    ...
    seq_rec_df = compute_motif_seq_rec_pass_rates(
        df, groupby_cols, motif_seq_rec_thresh
    )
    ...
    if custom_criteria is not None:
        criteria_to_run["custom_motif_success"] = OmegaConf.to_container(
            custom_criteria, resolve=True
        )
    ...
    for preset_name, criteria in criteria_to_run.items():
        ...
        rules = "  AND  ".join(
            f"{c['column']} {c['op']} {c['threshold']}" for c in resolved
        )
        ...
        for mode, models in thresholds.items():
            for model, spec in models.items():
                ...
                motif_region_filters.append(
                    {
                        "column": build_motif_region_column_name(
                            metric_type, mode, model
                        ),
                        "threshold": threshold,
                        "op": op,
                        "suffix": f"{prefix}_{mode}_{model}",
    ...
    # ------------------------------------------------------------------
    ss_ok = all(
        c in df.columns for c in ["_res_ss_alpha", "_res_ss_beta", "_res_ss_coil"]
    )
...
def run_motif_binder_analysis(
    ...
    if motif_binder_success is not None:
        motif_binder_success = OmegaConf.to_container(
            motif_binder_success, resolve=True
        )
        thresh_src = "custom"
    else:
    ...
    for seq_type, df_successful in successful_dfs.items():
        if len(df_successful) > 0:
            motif_binder_subsets.append(
                {"suffix": f"successful_{seq_type}", "df": df_successful}
            )

    subset_summary = ", ".join(
        f"{s['suffix']}({len(s['df'])})" for s in motif_binder_subsets
    )
...
def organize_results(results_dir: str) -> None:
    ...
    if moved_counts:
        summary = ", ".join(
            f"{subdir}/ ({count} files)" for subdir, count in moved_counts.items()
        )
...
def main(cfg: DictConfig) -> None:
    ...
    if result_type in [
        ...
        combined_df = merge_monomer_into_binder(
            combined_df, results_dir, config_name, input_mode
        )
    ...
    if result_type == "monomer_motif":
        combined_df = merge_monomer_into_motif(
            combined_df, results_dir, config_name, input_mode
        )
        ...
        try:
            if mode == "binder":
                ...
                if result_type not in binder_result_types:
                    logger.warning(
                        f"Skipping 'binder' analysis mode - not applicable for result_type={result_type}"
                    )
                    continue

                # Check if we have any binder-specific columns
                binder_cols = [
                    c for c in combined_df.columns if "complex_" in c or "binder_" in c
                ]
                if not binder_cols:
                    logger.warning(
                ...
                monomer_cols = [
                    ...
                    if "_res_scRMSD" in c or "_res_co_scRMSD" in c
                ]
                if not monomer_cols:
                    logger.warning(
                ...
                motif_binder_cols = [
                    c
                    for c in combined_df.columns
                    if "motif_rmsd_pred" in c or "motif_seq_rec" in c
                ]
                if not motif_binder_cols:
                    ...
                    failed_modes.append(
                        ("motif_binder", "No motif binder columns found")
                    )
                    continue
